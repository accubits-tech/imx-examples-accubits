...
async function main(ownerPrivateKey: string, network: string) {
  try {
    //Burn the asset
    console.log('burnNFT');
    const burnresponse = await burnNFT(
      ownerPrivateKey,
      '381',
      '0x19e81d345a3bb5194458b2df8ff49960c336b413',
      network,
    );
    ...
    if (getburnresponse.status == 'success') {
      console.log(
        ...
        getburnresponse.transaction_id,
      );

      //Attempt to mint an asset on the back of the burn
      const mintresponse = await mintV2(
        ownerPrivateKey,
        '2506',
        '0x19e81d345a3bb5194458b2df8ff49960c336b413',
        'none',
        '0xfaDcF1dEe4D008E02e9E97513081C320Ac2748B3',
        network,
      );
      ...
      if (mintfetch[0].status == 'success') {
        console.log(
          'Mint was successful, tx_id: ' + mintfetch[0].transaction_id,
        );
      } else {
