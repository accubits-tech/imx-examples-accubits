...
export default async (network : EthNetwork,privateKey:string, alchemyApiKey:string, collectionAddress:string, name:string,filterable:boolean ): Promise<void> => {
  try {
  ...
  const addMetadataSchemaToCollectionRes = await metadataApi.addMetadataSchemaToCollection(addMetadataSchemaToCollectionParams)
