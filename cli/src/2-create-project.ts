...
export default async (network:EthNetwork,privateKey:string, alchemyApiKey:string, projectName:string, companyName:string, contactEmail:string): Promise<void> => {
  try {
  ...
  const createProjectRes = await projectsApi.createProject(createProjectParams)
